...
const { addInventoryItemStockMovementDB } = require("./inventory.service");
...
exports.deleteInvoiceDB = async (
  ...
  try {
    ...
    // 1. Get all order IDs for this invoice
    const [orderRows] = await conn.query(
      `SELECT id FROM orders WHERE invoice_id = ? AND tenant_id = ?`,
      [invoiceId, tenantId]
    );
    const orderIds = orderRows.map(row => row.id);
    if (orderIds.length === 0) throw new Error("No orders linked to this invoice.");

    // 2. Get all order_items for these orders
    const [orderItems] = await conn.query(
      `SELECT id, item_id, variant_id, quantity FROM order_items
       WHERE order_id IN (?) AND tenant_id = ?`,
      [orderIds, tenantId]
    );

    if (orderItems.length === 0) throw new Error("No order items found.");

    // For combining all same inventory items together and update it in logs and items table in one go
    const inventoryRevertMap = new Map();

    // 3. For each order item, get linked inventory items from menu_item_recipes
    for (const item of orderItems) {
      const [recipeItems] = await conn.query(
        `SELECT inventory_item_id, quantity
         ...
         WHERE menu_item_id = ?
           AND addon_id = 0
           AND (variant_id = 0 OR variant_id = ?)
           AND tenant_id = ?`,
        [item.item_id, item.variant_id || 0, tenantId]
      );

      for (const recipe of recipeItems) {
        const totalQty = recipe.quantity * item.quantity;
        const existingQty = inventoryRevertMap.get(recipe.inventory_item_id) || 0;
        inventoryRevertMap.set(recipe.inventory_item_id, existingQty + totalQty);
      }
    }

    // Step 4: Revert inventory in a single call per inventory item
    for (const [inventoryItemId, totalQuantityToRevert] of inventoryRevertMap.entries()) {
      await addInventoryItemStockMovementDB(
        req,
        inventoryItemId,
        'IN',
        totalQuantityToRevert,
        `Invoice #${invoiceId} cancelled, reverting item usuage`,
        tenantId,
        username
      );
    }
